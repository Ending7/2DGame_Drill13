...
class Zombie:
    ...
    def move_opposite_boy(self, r=0.5):
        self.state = 'Walk'
        self.move_slightly_opposite(play_mode.boy.x, play_mode.boy.y)
        if self.distance_less_than(self.tx, self.ty, self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING

    def is_not_less_than_boys_ball(self):
        if self.ball_count >= play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL

    def less_than_boys_ball(self):
        if self.ball_count < play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('소년보다 공의 개수가 많거나 같은가?', self.is_not_less_than_boys_ball)
        a4 = Action('소년으로 이동', self.move_to_boy)

        SEQ_chase_boy = Sequence('소년을 추적',c1, c2, a4)

        c3 = Condition('소년보다 공의 개수가 적은가?',self.less_than_boys_ball)
        a5 = Action('소년에게서 도망', self.move_opposite_boy)
        SEQ_run_to_boy = Sequence('소년에게서 도망', c1, c3,a5)

        root = SEL_chase_or_wander = Selector('추적 또는 배회', SEQ_chase_boy, SEQ_run_to_boy, SEQ_wander)
